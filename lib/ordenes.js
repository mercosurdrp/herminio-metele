...
// (startDateFrom/startDateTo, error 409 si falta) — igual que maintenace-time,
// se pide por ventanas de hasta 364 días, paginando cada una.
export async function getOrdenes() {
  const key = apiKey();
  if (!key) return [];
  const mapaSuc = await fetchMapaSucursales(key);

  // Ventanas [desde, hasta] de 364 días desde ANIO_INICIO hasta hoy.
  const ventanas = [];
  let desde = new Date(Date.UTC(ANIO_INICIO, 0, 1));
  const hoy = new Date();
  while (desde <= hoy) {
    const hasta = new Date(Math.min(desde.getTime() + 364 * 86400000, hoy.getTime() + 86400000));
