...
// La API limita a 30 peticiones/min (compartidas con la página de checklists).
// Ante 429 esperamos lo que pida Retry-After (con tope) y reintentamos.
// `revalidate` usa el Data Cache de Vercel: las ventanas viejas no cambian,
// así que tras la primera carga casi no se consume el rate limit.
async function fetchCF(url, key, revalidate) {
  for (let intento = 0; ; intento++) {
    ...
    if (res.status !== 429 || intento >= 2) return res;
