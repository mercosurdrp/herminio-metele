...
async function fetchCF(url, key, revalidate) {
  for (let intento = 0; ; intento++) {
    ...
    if (res.status !== 429 || intento >= 2) {
      // Ritmo ≤27 req/min para dejar margen a los demás consumidores de la
      // cuenta. Si respondió en <150ms vino del Data Cache: no consumió cupo.
      if (Date.now() - t0 >= 150) await dormir(2200);
      return res;
    }
...
// Viene de a 50 por página: se sigue X-NextPage, si no los últimos vehículos
// del padrón quedan sin sucursal.
async function fetchMapaSucursales(key) {
  try {
    const m = {};
    let url = `${BASE}/vehicles/`;
    for (let pagina = 0; url && pagina < 20; pagina++) {
      const res = await fetchCF(url, key, 3600);
      if (!res.ok) break;
...
export async function getOrdenes() {
  ...
  // El listado de órdenes viene "liviano" (labors/parts vacíos y sin costos):
  // los trabajos y repuestos salen de los endpoints globales, unidos por
  // workOrderNumber.
  const [labors, parts] = [
    await fetchListaGlobal(key, "labors", ventanas),
    await fetchListaGlobal(key, "parts", ventanas),
  ];
  const porOrden = (arr) => {
    const m = new Map();
    for (const x of arr) {
  ...
  for (const o of crudas) {
    if (!o.labors?.length) o.labors = lm.get(o.number) || [];
    if (!o.parts?.length) o.parts = pm.get(o.number) || [];
  }

  // Ramallo es OTRA operación de la empresa: la flota Misiones es Eldorado +
  // Iguazú (se conservan órdenes sin sucursal por si falta el dato en la ficha).
  const esMisiones = (o) => {
    const suc = o.costCenter?.name || o.city?.name || mapaSuc[limpiarPatente(o.vehicleCode)] || null;
    return !suc || /^(eldorado|iguaz)/i.test(suc);
  };
...
// Lista global de trabajos o repuestos de todas las órdenes, por las mismas
// ventanas de fecha del listado. OJO: acá los parámetros llevan prefijo "wo"
// (woStartDateFrom/woStartDateTo = fecha de la orden) y el rango máximo es
// 6 meses — con startDateFrom a secas responde 409 como si faltara el rango.
async function fetchListaGlobal(key, recurso, ventanas) {
  const porId = new Map();
  for (const [vDesde, vHasta] of ventanas) {
    const esUltima = vHasta === ventanas[ventanas.length - 1][1];
    const revalidate = esUltima ? 300 : 86400;
    let url = `${BASE}/work-orders/${recurso}/?woStartDateFrom=${vDesde}&woStartDateTo=${vHasta}`;
    for (let pagina = 0; url && pagina < MAX_PAGINAS; pagina++) {
      const res = await fetchCF(url, key, revalidate);
      if (!res.ok) break; // best effort: sin esto igual mostramos las órdenes
      const data = await res.json();
      const arr = Array.isArray(data) ? data : data.items || data.data || [];
      for (const x of arr) porId.set(x.id ?? `${x.workOrderNumber}|${x.name}`, x);
      if (!arr.length) break;
      const next = res.headers.get("X-NextPage") || res.headers.get("x-nextpage");
      url = next ? next.replace(/^"|"$/g, "") : null;
    }
