...
// Combustible — dashboard de cargas de Cloudfleet (fuel-entries):
// consumo por camión y por chofer, km/litro, costos, filtros por
// sucursal / año / mes / día, gráfico de columnas y PDA.
...
// Autoelevadores: en Cloudfleet cargan con códigos TOYOTA1, TOYOTA2, …
...
// Métricas del gráfico de columnas.
const METRICAS = [
  { key: "litros", label: "Litros", color: "#1d4ed8", fmt: (v) => `${fmtCorto(v)} L` },
...
export default function Combustible() {
  ...
  const [progreso, setProgreso] = useState(null); // texto mientras se arma el histórico
    ...
    const pedir = async (seguir) => {
      try {
        ...
        if (j.parcial && pedidos.current < 6) {
          // Primera vez: el histórico se arma por tramos; seguimos pidiendo.
          pedidos.current += 1;
  ...
  // Base = filtros de sucursal + período aplicados.
  const base = useMemo(
    () =>
      entradas.filter((e) => {
  ...
  // Tarjetas resumen (solo camiones; los autoelevadores tienen su sección).
  const resumen = useMemo(() => {
    ...
    return {
      ...
      costo,
      km,
      kmxl: litros > 0 ? km / litros : null,
      costoLitro: litros > 0 ? costo / litros : null,
      cargas: camiones.length,
    };
  }, [camiones]);
  ...
  // Serie del gráfico: por mes (o por día si hay mes elegido). Camiones + autoelevadores.
  const serieCol = useMemo(() => {
    ...
    for (const e of base) {
      ...
      if (!m.has(clave)) m.set(clave, { clave, litros: 0, costo: 0, cargas: 0, km: 0 });
      const g = m.get(clave);
      g.litros += e.litros || 0;
      g.costo += e.costo || 0;
      g.km += e.km || 0;
  ...
  return (
    <main className="wrap">
      ...
      <div className="muted" style={{ marginBottom: "1rem" }}>
        <small>
          Cargas de combustible de Cloudfleet, año 2026 (GasOil / Nafta / GNC; urea y aceite quedan afuera).
          Camiones de la flota vigente + autoelevadores.
          {data?.actualizado && (
        ...
        <>
          ...
          <div className="cards">
            ...
            <div className="card">
              ...
              <div className="sub">combustible camiones</div>
            </div>
            <div className="card">
          ...
          <div className="chart-card" style={{ marginTop: 0, marginBottom: "1.5rem" }}>
            <div className="chart-head">
              <h2>
                {met.label} de combustible {mes ? `· ${etiquetaMes(mes)} (por día)` : "(por mes)"}
                {sucursal ? ` · ${sucursal}` : ""}
              </h2>
            ...
            <div className="muted" style={{ fontSize: "0.85rem", marginBottom: "0.4rem" }}>
              Camiones + autoelevadores. Elegí un mes arriba para abrirlo por día.
            </div>
                  ...
                  return (
                    ...
                    >
                      <div className="bars">
                        <div
                          className="bar"
                          style={{ height: `${(g[metrica] / serieCol.max) * 100}%`, background: met.color }}
                        >
            ...
            {tipCol && (
              <div className="adh-tooltip" style={{ left: tipCol.x, top: tipCol.y }}>
                <div className="tt-title">{etiquetaCol(tipCol.g.clave)}</div>
          ...
          {/* Autoelevadores Toyota */}
          <div className="chart-card" style={{ marginTop: 0, marginBottom: "1.5rem" }}>
            <div className="chart-head">
              <h2>Autoelevadores Toyota ({porAutoelevador.length})</h2>
            </div>
            <div className="muted" style={{ fontSize: "0.85rem", marginBottom: "0.7rem" }}>
              Consumo medido por horímetro: litros por hora de uso · período: {periodoTexto}.
            </div>
